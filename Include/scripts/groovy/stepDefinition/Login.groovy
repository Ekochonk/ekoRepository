...
public void user_input_registered_username_only(String username) {
	...
	WebUI.callTestCase(findTestCase('Pages/User Login/Input Password'), [('password') : ''], FailureHandling.STOP_ON_FAILURE)
}

@Then("User input registered password only {string}")
public void user_input_registered_password_only(String password) {
	WebUI.callTestCase(findTestCase('Pages/User Login/Input Username'), [('username') : ''], FailureHandling.STOP_ON_FAILURE)
